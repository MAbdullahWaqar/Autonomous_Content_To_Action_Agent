...
const schemas_1 = require("../src/lib/agents/schemas");
const prompts_1 = require("../src/lib/agents/prompts");
const outcome_evidence_1 = require("../src/lib/agents/outcome-evidence");
...
function runTests() {
    return __awaiter(this, void 0, void 0, function* () {
        const model = (0, google_1.google)(MODEL_ID);
        const logStream = fs_1.default.createWriteStream(path_1.default.join(__dirname, 'test-results.md'), { flags: 'w' });
        const log = (msg) => {
            console.log(msg);
            logStream.write(msg + '\n');
        };
        ...
        const criticResult = yield (0, ai_1.generateObject)({
            model,
            schema: schemas_1.ActionCriticSchema,
            prompt: prompts_1.AGENT_PROMPTS.actionCritic + badActionContext,
        });
        ...
        const promptWithRetry = prompts_1.AGENT_PROMPTS.executionSimulator + baseContext + prompts_1.SIMULATION_RETRY_SUFFIX;
        log('**Simulating Retry Round 2 (with Retry Suffix)...**\n');
        const simResult = yield (0, ai_1.generateObject)({
            model,
            schema: schemas_1.SimulationSchema,
            prompt: promptWithRetry,
        });
        const simulation = simResult.object;
        const validation = (0, outcome_evidence_1.validateSimulation)(simulation);
